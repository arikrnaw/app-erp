<template>
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold tracking-tight">Edit Bill of Materials</h1>
                <p class="text-muted-foreground">
                    Update BOM details and component structure
                </p>
            </div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        <Checkbox
                                            ...
                                            :id="`critical-${index}`"
                                        />
                                        <Label :for="`critical-${index}`">Mark as critical</Label>
                                    </div>
                                </div>
                                <div class="space-y-2">
...
import type { Product, BomItem, BillOfMaterial } from '@/types/erp';
...
const fetchBom = async () => {
    ...
    try {
        ...
        form.value = {
            bom_number: response.bom_number,
            name: response.name,
            description: response.description || '',
            product_id: response.product_id.toString(),
            quantity_per_unit: response.quantity_per_unit,
            unit: response.unit,
...
const handleSubmit = async () => {
    ...
    try {
        ...
        await apiService.updateBillOfMaterial(props.id, submitData);
        router.visit(`/manufacturing/bill-of-materials/${props.id}`);
    } catch (error: any) {
