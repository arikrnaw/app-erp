<template>
    ...
    <Head title="Edit Report Schedule" />
    <AppLayout>
        <template #header>
        ...
        <div class="space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold tracking-tight">Edit Report Schedule</h1>
                    <p class="text-muted-foreground">
                        Update the report schedule: {{ schedule.name }}
                    </p>
                </div>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Update the schedule details and delivery settings.
                    </CardDescription>
                </CardHeader>
                <CardContent>
...
const form = useForm({
    ...
    report_type: props.schedule.report_type,
    report_template: props.schedule.report_template || '',
    frequency: props.schedule.frequency,
    day_of_week: props.schedule.day_of_week || '',
    day_of_month: props.schedule.day_of_month || '',
    delivery_time: props.schedule.delivery_time,
    delivery_method: props.schedule.delivery_method,
    recipients: props.schedule.recipients,
    parameters: props.schedule.parameters,
    is_active: props.schedule.is_active,
});

const submit = () => {
    form.put(route('reports.schedules.update', props.schedule.id), {
        onSuccess: () => {
            // Handle success
        },
    });
...
const breadcrumbs = [
    ...
    { name: props.schedule.name, href: route('reports.schedules.show', props.schedule.id) },
    { name: 'Edit Schedule', href: '#' },
];
</script>
