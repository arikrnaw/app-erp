    ...
    <template #header>
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-4">
          <Link :href="route('finance.accounts-receivable.invoices.show', invoice.id)"
            ...
            <form @submit.prevent="submit" class="space-y-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <Input id="invoice_date" v-model="form.invoice_date" type="date" :disabled="loading" />
                ...
                <div v-if="form.reference_type">
                  ...
                  <Input id="reference_id" v-model="form.reference_id" type="number" placeholder="Enter reference ID"
                    :disabled="loading" />
              ...
              <div>
                <div class="flex items-center justify-between mb-4">
                  ...
                  <Button type="button" @click="addItem" :disabled="loading" variant="outline" size="sm">
                  ...
                  <div v-for="(item, index) in form.items" :key="index" class="border rounded-lg p-4">
                    <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
                      ...
                      <div>
                        ...
                        <Input :id="`quantity_${index}`" v-model="item.quantity" type="number" step="0.01" min="0.01"
                          placeholder="0.00" :disabled="loading" @input="calculateItemTotal(index)" />
                      </div>

                      <div>
                        <Label :for="`unit_price_${index}`">Unit Price</Label>
                        <Input :id="`unit_price_${index}`" v-model="item.unit_price" type="number" step="0.01" min="0"
                          placeholder="0.00" :disabled="loading" @input="calculateItemTotal(index)" />
                      </div>

                      <div class="flex items-end space-x-2">
                        <Button type="button" @click="removeItem(index)" :disabled="loading || form.items.length <= 1"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
                      <div>
                        ...
                        <Input :id="`description_${index}`" v-model="item.description" placeholder="Item description"
                          :disabled="loading" />
                      </div>

                      <div>
                        <Label :for="`tax_rate_${index}`">Tax Rate (%)</Label>
                        <Input :id="`tax_rate_${index}`" v-model="item.tax_rate" type="number" step="0.01" min="0"
                          max="100" placeholder="0.00" :disabled="loading" @input="calculateItemTotal(index)" />
                      </div>

                      <div>
                        <Label :for="`discount_rate_${index}`">Discount Rate (%)</Label>
                        <Input :id="`discount_rate_${index}`" v-model="item.discount_rate" type="number" step="0.01"
                          min="0" max="100" placeholder="0.00" :disabled="loading" @input="calculateItemTotal(index)" />
                      </div>
...
const form = useForm({
  ...
  reference_type: props.invoice.reference_type || '',
  reference_id: props.invoice.reference_id || '',
  ...
  items: props.invoice.items?.map(item => ({
    ...
    discount_rate: parseFloat(item.discount_rate.toString()),
...
const submit = () => {
  ...
  form.put(route('finance.accounts-receivable.invoices.update', props.invoice.id))
}
